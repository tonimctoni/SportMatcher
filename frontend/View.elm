...
start_poll model =
  ...
  [ h1 [style [("margin", ".2cm")]] [text "Start Survey"]
  , div [class "col-md-12", style [("margin", ".2cm"), ("padding", ".2cm"), ("border", ".5px solid red"), ("border-radius", "4px")]]
    [ div [class "row"] [div [class "col-md-4"] [input [type_ "text", placeholder "#Responders", onInput UpdateNumber, value (if model.start_poll_form_number==0 then "" else toString model.start_poll_form_number)] []]]
    , div [class "row"] [div [class "col-md-4"] [input [type_ "text", placeholder "Title", onInput UpdateTitle, value model.start_poll_form_title] []]]
    , div [class "row"]
      [ div [class "btn-group col-md-4", style [("margin-top", ".2cm")]]
        [ button [class "btn btn-primary", disabled (not model.start_poll_form_type_is_free), onClick ClickedFixed] [text "Fixed"]
        , button [class "btn btn-primary", disabled model.start_poll_form_type_is_free, onClick ClickedFree] [text "Free"]
        ]
      ]
    , if model.start_poll_form_type_is_free
        then div [] []
        else div [class "row"] [div [class "col-md-4"] [textarea [rows 10, style [("margin-top", ".2cm"), ("margin-bottom", ".2cm")], onInput UpdateQuestions, value model.start_poll_form_questions] []]]
    , div [class "row"] [div [class "col-md-4"] [button [onClick ClickedSubmitPoll] [text "Submit"]]]
    , div [] [if String.length model.error>0 then p [style [("font-weight", "bold"), ("color", "red")]] [text model.error] else div [] []]
    ]
  ]
...
view model =
  ...
  , div []
    [ case model.page of
        StartPollPage -> start_poll model
        ShowPollLinkPage -> div [] [text model.poll_id]
    ]
  ]
